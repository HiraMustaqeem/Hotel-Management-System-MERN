...
const currentHour = new Date().getHours();
 ...
 {/* ─── HEADER PANEL SECTION ─── */}
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-slate-200/60 mt-3">
        <div className="w-full bg-slate-900 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="absolute right-0 top-0 w-80 h-80 bg-[rgb(94,74,247)]/10 rounded-full blur-3xl pointer-events-none" />
        
        <div className="space-y-1.5 relative z-10">
          <span className="text-[10px] uppercase font-bold tracking-widest text-[rgb(94,74,247)] bg-[rgb(94,74,247)]/10 px-3 py-1 rounded-full border border-[rgb(94,74,247)]/20">
            Live Workspace Matrix
          </span>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight pt-2">
            {greetingText}, Luxury Stay!
          </h1>
          <p className="text-slate-400 text-xs md:text-sm max-w-xl font-medium">
            Welcome back to the central operational terminal. Here is your real-time estate snapshot index for today.
          </p>
        </div>

        <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-md self-start md:self-auto">
          <div className="w-3 h-3 bg-emerald-500 rounded-full animate-ping" />
          <div className="text-xs">
            <span className="text-slate-400 block font-bold uppercase text-[9px] tracking-wider">Terminal Connection</span>
            <span className="font-extrabold text-slate-100">Server Status: Online</span>
          </div>
        </div>
      </div>

        {/* Filter Configuration Switcher */}
        <div className="inline-flex rounded-xl p-1 bg-slate-200/60 border border-slate-200 self-start md:self-center">
          <img src="/" alt="" />
        </div>
      </div>
